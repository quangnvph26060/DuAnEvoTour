  ...
  $.HSCore.components.HSChartMatrixJS = {
    ...
    init: function (el, options) {
      ...
      var context = this,
        ...
        dataSettings = el.attr("data-hs-chartjs-options")
          ? JSON.parse(el.attr("data-hs-chartjs-options"))
          : {},
        settings = {};
      ...
      if (settings.options.hasOwnProperty("matrixBackgroundColor")) {
        settings.data.datasets.forEach(function (datasets) {
          datasets.backgroundColor = function (ctx) {
            ...
            var additionToValue =
              settings.options.matrixBackgroundColor.hasOwnProperty(
                "additionToValue"
              )
                ? settings.options.matrixBackgroundColor.additionToValue
                : 5;
            var alpha =
              (additionToValue + value) /
              settings.options.matrixBackgroundColor.accent;

            if (
              value.toFixed() == 0 &&
              settings.options.matrixBackgroundColor.hasOwnProperty("nullColor")
            ) {
              return Color(
                settings.options.matrixBackgroundColor.nullColor
              ).rgbString();
            } else {
              return Color(settings.options.matrixBackgroundColor.color)
                .alpha(alpha)
                .rgbString();
            }
          };
      ...
      if (settings.options.hasOwnProperty("matrixLegend")) {
        ...
        for (var i = 1; i < settings.data.datasets[0].data.length; i++) {
          if (settings.data.datasets[0].data[i].v < min)
            min = settings.data.datasets[0].data[i].v;
          if (settings.data.datasets[0].data[i].v > max)
            max = settings.data.datasets[0].data[i].v;
        }
        ...
        var html = [],
          stepSize = settings.options.matrixLegend.hasOwnProperty("stepSize")
            ? settings.options.matrixLegend.stepSize
            : max / 10,
          additionToValue =
            settings.options.matrixBackgroundColor.hasOwnProperty(
              "additionToValue"
        ...
        $(settings.options.matrixLegend.container).addClass(
          "hs-chartjs-matrix-legend"
        );

        $(settings.options.matrixLegend.container).append(
          '<li class="hs-chartjs-matrix-legend-min">' + min + "</li>"
        );
        ...
        for (var i = 0; i < max; ) {
          ...
          html.push(
            '<li class="hs-chartjs-matrix-legend-item" style="background-color: ' +
              Color(settings.options.matrixBackgroundColor.color)
                .alpha(alpha)
                .rgbString() +
              '"></li>'
          );
        ...
        $(settings.options.matrixLegend.container).append(
          '<li class="hs-chartjs-matrix-legend-max">' + max + "</li>"
        );
      }
